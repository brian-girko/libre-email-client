...
const TTL = {
  epoch: {bodies: Infinity, threads: 60 * 60 * 1000},
  short: {bodies: 15 * 60 * 1000, threads: 15 * 60 * 1000},
  keys: {bodies: Infinity, threads: Infinity},
};
...
export class MailCache {
  ...
  // kind ('bodies' | 'threads'; undefined = both). Runs after every put and
  // once at open.
  async sweep(kind) {
    try {
      await this.ready;
      const caps = {bodies: MAX_BODIES, threads: MAX_THREADS};
      const kinds = kind ? [kind] : ['bodies', 'threads'];
