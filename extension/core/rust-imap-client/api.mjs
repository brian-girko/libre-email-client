...
import { MailCache } from './cache.mjs';
 ...
 * @property {string} [accountId] stable account id; scopes cache entries.
 *   Defaults to "user@host" when omitted.
 * @property {"epoch"|"short"|"keys"} [cachePolicy="epoch"] expiry policy:
 *   epoch = bodies never expire (content-immutable), threads/dirs 1h backstop;
 *   short = everything 15 minutes; keys = no time expiry, correctness via
 *   uidvalidity/uidnext keys + action invalidation.
 */
 ...
 * @property {() => Promise<Folder[]>} listDirs list of mailboxes. Cached per
 *   account (one entry) until createDir/deleteDir or the cache policy TTL.
 ...
 *   drops its cached threads and closes it when it was the open dir.
 ...
 *   newest thread first). Results are cached per dir until openDir/setFlags/
 *   moveTo runs again; {refresh: true} bypasses both cache tiers and
 *   refetches the folder from the server, re-populating the caches.
 ...
 * @property {() => string} cacheBackend active cache backend name
 *   ("CacheStorage" in the browser, "memory" fallback).
 * @property {() => Promise<void>} clearCache drop every cached entry
 *   (bodies + threads) of this account.
...
export async function createMailApi(cfg) {
    ...
    let selected = null;
    let status = null;
    let threadCache = null; // ThreadSummary[] of `selected`, or null when stale
    ...
    // Persistent two-tier cache (CacheStorage in the browser, memory in
    // Node): bodies are content-immutable per (accountId, uidvalidity, uid),
    // threads are keyed by uidnext and evicted on mutating actions.
    const cache = new MailCache({
        policy: cfg.cachePolicy,
        accountId: cfg.accountId || `${cfg.user}@${cfg.host}`,
        log,
    });
    ...
    return {
        ...
        async listDirs() {
            ...
            const t0 = Date.now();
            try {
                // dirs tier: one entry per account, evicted on createDir/
                // deleteDir and time-expired per cache policy, so a cached
                // list is never shown while it can still be locally wrong
                const hit = await cache.get('dirs', []);
                if (hit) {
                    const out = JSON.parse(new TextDecoder().decode(hit));
                    log(`listDirs() -> ${out.length} dirs (cache, ${Date.now() - t0}ms)`);
                    return out;
                }
                ...
                if (!out.length) {
                    ...
                    // mail.mjs wrapper reconnects and retries on a fresh
                    // session instead of showing a bogus empty folder pane.
                ...
                await cache.put('dirs', [], new TextEncoder().encode(JSON.stringify(out)));
        ...
        async listThreads(opts) {
            ...
            try {
                ...
                if (hit) {
                    try {
                        threadCache = JSON.parse(new TextDecoder().decode(hit));
                        log(`listThreads() -> ${threadCache.length} threads (cache, ${Date.now() - t0}ms)`);
                        return threadCache;
                    } catch {
                ...
                threadCache = rows.map(mapThread);
                await cache.put('threads', cacheKey, new TextEncoder().encode(JSON.stringify(threadCache)));
                log(`listThreads() -> ${threadCache.length} threads (${Date.now() - t0}ms)`);
                return threadCache;
            } catch (e) {
        ...
        async readFile(uid) {
            ...
            try {
                // bodies are content-immutable per (uidvalidity, uid) epoch:
                // flags/moves never invalidate them, so cache-first always
                const cacheKey = [selected, String(Number(status?.uidvalidity) || 0), String(uidNum)];
                const hit = await cache.get('bodies', cacheKey);
                if (hit) {
                    log(`readFile(${uid}) -> ${hit.length} bytes (cache, ${Date.now() - t0}ms)`);
                    return hit;
                }
                const raw = await clientCall('fetch_message', [uidNum]);
                const bytes = raw instanceof Uint8Array ? raw : new Uint8Array(raw);
                await cache.put('bodies', cacheKey, bytes);
        ...
        async setFlags(uids, addFlags, removeFlags) {
            ...
            threadCache = null; // flag aggregates inside thread summaries
            cache.invalidateThreads(selected); // persistent tier follows
            const t0 = Date.now();
            try {
        ...
        async moveTo(uids, mailbox) {
            ...
            threadCache = null;
            cache.invalidateThreads(selected);
            const t0 = Date.now();
            try {
        ...
        // cache introspection/maintenance (options page, tests)
        cacheBackend() {
            return cache.backendName;
        },
