...
//
// Filtered mail is marked with the IMAP keyword $Filtered (best-effort).
// Flags survive MOVE/COPY, so a message the user manually moves back to
// INBOX carries the mark and every pass skips it instead of filtering it
// away again. A skipped message counts as processed (the watermark advances
// past it); the manual run can opt out of the skip via a checkbox on the
// options page. Servers without custom-keyword support degrade to the old
// behavior: the flag step fails silently and mail can be re-filtered.
//
...
import {createMailApi} from '../rust-imap-client/api.mjs';
...
// IMAP keyword marking messages the filters already acted on. Flags travel
// with MOVE/COPY, so the mark follows the message when the user moves it
// back to INBOX and applyFilters skips it there (see FILTERED_FLAG use).
const FILTERED_FLAG = '$Filtered';
...
async function applyToUnread({api, accountId, parsed, entry, uidvalidity, trackWatermark, skipFiltered = true, onProgress}) {
  const pageSize = 500;
  const all = [];
  for (let page = 0; ; page++) {
  ...
  const unread = all.filter(m => !m.flags.includes('\\Seen') &&
    !(skipFiltered && m.flags.includes(FILTERED_FLAG)));
  unread.sort((a, b) => a.uid - b.uid);
  let failedUid = null;
  let lastGood = null;
  let done = 0;
  for (const summary of unread) {
    if (!await applyFilters(api, parsed, summary, entry, skipFiltered)) {
...
// skipFiltered (default true) skips messages marked with the $Filtered
// keyword — mail the filters already handled, e.g. brought back into INBOX
// by hand; the manual run's checkbox can opt out.
export async function runAccountFilters({api, accountId, filters, entry, scope, skipFiltered = true, onProgress}) {
  ...
  // automatic runs keep treating exactly the same mail as new. $Filtered
  // mail is skipped unless the caller opted out (skipFiltered: false). On a
  ...
  if (scope === 'unread') {
    await applyToUnread({api, accountId, parsed, entry, uidvalidity, trackWatermark: false, skipFiltered, onProgress});
    return entry;
  }
  ...
  for (const summary of newMessages) {
    const processed = await applyFilters(api, parsed, summary, entry, skipFiltered);
    if (processed) {
...
// Returns true when the message was fully processed (acted, no match, or
// already marked $Filtered and skipped), false on error (caller stops and
// retries next run).
async function applyFilters(api, parsed, summary, entry, skipFiltered = true) {
  // already handled once (the $Filtered mark follows the message through
  // MOVE/COPY): skip without acting, but count as processed so the watermark
  // still advances past it
  if (skipFiltered && summary.flags.includes(FILTERED_FLAG)) {
    return true;
  }
  for (const {filter, ast} of parsed) {
    try {
      ...
      if (filter.action === 'move') {
        ...
        // mark before the move so the keyword travels with the message
        // (MOVE/COPY preserves flags); on a failed move, un-mark again so
        // the retry path re-processes the message
        try {
          await api.setFlags([summary.uid], [FILTERED_FLAG], []);
        }
        catch {}
        try {
        ...
        // mark only after the copy succeeded: a failed save must leave the
...
// Apply every enabled filter to its account(s), one short-lived IMAP
// session per account (refcounted bridge access), so it never interferes
// with the user's live client session. scope 'unread' (the options page's
// "Run filters now" button) sweeps all unread INBOX mail instead of the
// watermark's new messages; skipFiltered (default true) skips mail already
// marked $Filtered — the manual run's checkbox can opt out. Returns the run
// result:
//   {time, accounts: [{id, label, moved, deleted, errors}], total}
export async function runFilters({scope, skipFiltered = true, onAccountStart, onAccountEnd, onAccountProgress} = {}) {
  ...
  for (const accountId of perAccount.keys()) {
    result.accounts.push(await runAccountSession(
      ...
      {cachePolicy, debug, scope, skipFiltered, onAccountStart, onAccountEnd, onAccountProgress}
    ));
  }
...
// One short-lived IMAP session for one account; the bridge is always
// released. Account-level problems (missing config/bridge) land in
// entry.errors so the status surfaces stay informative; a missing password
// is not an error: the account is skipped silently and the storage listener
// re-runs the pass once a password lands in session storage.
async function runAccountSession(accountId, accountFilters, accounts, {cachePolicy, debug, scope, skipFiltered = true, onAccountStart, onAccountEnd, onAccountProgress}) {
  const entry = {
  ...
  try {
    ...
    const bridgeUrl = await bridgeRequest();
    bridgeAcquired = true;
    if (!bridgeUrl) {
      entry.errors.push('IMAP bridge unavailable');
      return entry;
    }
    api = await createMailApi({
      ...
      host,
      port,
      secure: cfgRes[fieldKey('imap.secure', accountId)] !== false,
      allowSelfSigned: !!cfgRes[fieldKey('imap.allowSelfSigned', accountId)],
      user,
      pass,
      accountId,
      cachePolicy,
    ...
    await api.connect();
    await runAccountFilters({api, accountId, filters: accountFilters, entry, scope, skipFiltered, onProgress: onAccountProgress});
  }
  catch (e) {
