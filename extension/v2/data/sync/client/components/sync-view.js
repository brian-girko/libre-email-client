...
// and combo-wrapped Run / Dry run filter buttons), the Discard local copy
// button and the Stop button (kills the ongoing sync — the engine document
// is closed and every queued job dropped) — plus the log pane fed by the
// engine's log stream.
//
// Dumb renderer: it forwards 'sync-request' CustomEvents
// {kind:'sync'|'dry'|'sync-dir'|'dry-dir'|'discard'|'filter-dir'|
// 'dry-filter-dir', account, dir[, filterId, scope]} and exposes setters
// consumed by the wiring in the client/sync-panel.mjs.
  ...
  constructor() {
    ...
    this.#shadow.innerHTML = `
      ...
      .row[hidden] {
        ...
        #discard, #kill { cursor: pointer; }
        #discard:hover:enabled, #kill:hover:enabled { border-color: var(--dim, #6f747d); }
        #discard:disabled, #kill:disabled, select:disabled {
          opacity: 0.45;
          cursor: not-allowed;
        }
        .dirbox {
