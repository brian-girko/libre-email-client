...
class ListView extends HTMLElement {
  ...
  // override ran without its delete checkbox). Resolves 'deleted' (the
  // local trash-flag rename — the IMAP purge replays at the next sync),
  ...
  askAfterSave(count, label) {
    ...
    const turn = this.#afterSaveQueue.then(() => new Promise(resolve => {
      ...
      this.#afterSaveSub.textContent =
        'What should happen to the original copies? "Move" stages them into this server\u2019s ' + label +
        ' folder (both actions replay to the server at the next sync).';
