...
// the folder re-renders from the local copy once the engine's post-op sync
// removes them server-side. Failures surface as a job error — no snapshot
// rollback to undo.
...
// reconciled by the folder re-render that follows the op's resync. No
// counter prediction: the tree badges + title update when the sync lands.
// Quiet jobs render no bar line (auto mark-read on open).
...
// Override flow as a single multi-stage job: the raw copies are written to
// disk first (nothing leaves the visible list yet — the originals are still
// on the server), then the server side runs — auto-delete, or the
// askAfterSave modal deciding between delete / move / leave-on-server. Rows
// never leave the view optimistically: the folder re-renders from the local
// copy when the delete/move's resync removes them for real. Server problems
// are re-thrown with the save outcome prefixed so the user can tell copy
// from delete problems.
async function runSaveAction(action, override, spec, id, name, uids, token) {
  ...
  enqueue({
    ...
    run: async (api, job) => {
      ...
      // failed emails remain harmless on the server: their rows never left
      // the view, so nothing needs restoring — every failure is logged
      if (failed.length) {
      ...
      if (choice === 'deleted') {
        try {
          // local-first purge: the T (trash) flag rename lands instantly
          // (offline OK); the IMAP expunge replay is diff-driven by the
          // engine's sync pass from the filename flags
          await api.deleteMessages(copied);
          job.label = `Saved ${saved} to ${dir} and deleted from the local copy — the IMAP purge replays at the next sync${failures}`;
        }
        catch (e) {
          throw new Error('Saved ' + saved + ' to ' + dir + ', but the local delete failed: ' + (e?.message || e));
        }
      }
      else if (choice === 'move') {
        try {
          ...
          job.label = `Saved ${saved} to ${dir} and moved to ${target} — the server move replays at the next sync${failures}`;
        }
        catch (e) {
          throw new Error('Saved ' + saved + ' to ' + dir + ', but the local move failed: ' + (e?.message || e));
        }
      }
      else {
...
function init(element) {
  ...
  el.addEventListener('refresh', async () => {
    // toolbar refresh = re-render the mails view from the local copy only:
    // no server request, the engine's periodic/client-open syncs fill the
