...
// without an extra round-trip before a move.
...
// column. Under the mirror architecture this is a pure local read: every
// folder's summary lives in _meta.json (maintained by the SyncEngine's
// syncs — the only writer of the mirror), so the whole sweep answers
// instantly and the per-folder reconciles still stream through the counter
// manager.
...
async function countDirs(api, id, token) {
  try {
    ...
    // no mirror yet (first open before the engine's first sync): the column
    // simply stays empty and fills with the next sync's mirror-changed
  }
}
...
async function load(id) {
  ...
  el.loading();
  if (!id) {
    // No accounts configured (empty select): dead-ending on "No account
    // selected" would leave a first-time user stranded — the pane must point
    // them at the options page (the "Open Options" offer) where accounts are
    // added. A missing bridge additionally keeps its own setup offer.
    if (await setupIncomplete()) {
      ...
      el.optionsNeeded('No accounts yet — add your first account on the options page.');
    }
    return;
  }
  try {
    ...
    // Bring the mirror up to date in the background: the tree/list render
    // from the current local state immediately, and the SyncEngine's
    // mirror-changed events fold fresh data in as it lands.
    chrome.runtime.sendMessage({type: 'sync-now', accountId: id, reason: 'client_open'}).catch(() => {});
...
function init(element) {
  ...
  // Mirror updates (engine syncs, replayed ops, post-op resyncs) refresh
  // the tree in place: re-read the folder list + counts from the mirror.
  mirrorChanged.subscribe(async evt => {
    ...
    /* transient mirror read failure: the next event retries */
  }
});
}
