...
// Ask the service worker to recount the badge from the stored local copy
// after unread-affecting work lands. Trailing debounce so a burst of actions
// (multi-select or the quiet auto mark-read on open) coalesces into one
// recount. The recount is mirror-only (the engine resynced the touched
// folders before the op's response arrived, so the local copy is already
// current) and updates the icon silently.
...
// lanes are idle, so serialized op bursts never interleave with a save.
...
function process(accountId) {
  ...
  // lanes are idle so serialized ops never interleave with a save
