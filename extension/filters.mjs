...
//
//   - 'filters-check' runtime messages (the options page "Run filters now"
//     button and the mail client opening): run a standalone pass and answer
//     with the result — no client page needs to be open anymore. The button
//     may request scope 'unread' (a one-off sweep over all unread INBOX
//     mail); every other caller runs the incremental new-mail pass.
...
async function doRun(scope, skipFiltered) {
...
// options page's one-off all-unread sweep). skipFiltered (default true)
// skips mail already marked $Filtered so a message manually moved back into
// INBOX is never filtered again; the manual run's checkbox opts out. A run
// already in flight wins: concurrent callers share its result regardless of
// scope.
export function runFilters(scope, skipFiltered = true) {
  if (running) {
    return running;
  }
  running = withLock(() => doRun(scope, skipFiltered))
    .catch(e => {
...
// Badge pre-pass: apply this account's filters inside the badge's own IMAP
// session, then run the badge's count in the SAME critical section, so mail a
// rule is about to move never reaches the badge count and no standalone pass
// can move mail between the filter step and the count. `count(entry)` is the
// caller's count callback; it receives the filter outcome (or null when no
// filter applies) and its return value is handed back as `counted`.
// Returns {entry, counted}; entry is null when no filter applies.
export async function runForBadge({api, accountId, label, count}) {
  const filters = await filtersForAccount(accountId);
  const out = await withLock(async () => {
    let entry = null;
    if (filters.length) {
      entry = await applyForBadge({api, accountId, label, filters, onProgress: progressEmitter({requireAction: true})});
    }
...
// Badge pre-pass body (caller holds the filter mutex via runForBadge):
// a periodic pass stays silent unless it actually moved/deleted mail or hit
// an error, so the logger only reports real filter activity.
async function applyForBadge({api, accountId, label, filters, onProgress}) {
  const entry = {id: accountId, label, moved: 0, deleted: 0, errors: [], changes: []};
  await runAccountFilters({api, accountId, filters, entry, onProgress});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // skipFiltered defaults to true: only an explicit false (the manual run's
  // "re-evaluate everything" checkbox) re-processes $Filtered mail.
  runFilters(message.scope === 'unread' ? 'unread' : 'new', message.skipFiltered === false ? false : true)
