...
const cachePolicyEl = document.getElementById('f-cache-policy');
...
async function loadGlobalPrefs() {
  ...
  cachePolicyEl.value = ['epoch', 'short', 'keys'].includes(res['ui.cachePolicy'])
    ? res['ui.cachePolicy']
    : 'epoch';
...
saveGlobalBtn.addEventListener('click', async () => {
  ...
  await chrome.storage.local.set({
    ...
    'ui.cachePolicy': ['epoch', 'short', 'keys'].includes(cachePolicyEl.value)
      ? cachePolicyEl.value
      : 'epoch',
...
async function refreshBadgeStatus() {
  const {['badge.last']: last} = await chrome.storage.session.get('badge.last');
  renderBadgeStatus(last);
}

chrome.storage.onChanged.addListener((changes, area) => {
  if (area === 'session' && 'badge.last' in changes) {
...
const filterSkipEl = document.getElementById('f-filter-skip');
...
runFiltersBtn.addEventListener('click', async () => {
  ...
  try {
    const response = await chrome.runtime.sendMessage({
      ...
      skipFiltered: filterSkipEl.checked
    });
    if (response?.ok && response.result) {
...
// same for the "$Filtered skip" toggle of the manual run
filterSkipEl.addEventListener('change', async () => {
  await chrome.storage.local.set({'filters.skipFiltered': filterSkipEl.checked});
});

async function loadFiltersPrefs() {
  const res = await chrome.storage.local.get({filters: [], 'filters.runScope': 'unread', 'filters.skipFiltered': true});
  filters = Array.isArray(res.filters) ? res.filters : [];
  filterScopeEl.value = res['filters.runScope'] === 'new' ? 'new' : 'unread';
  filterSkipEl.checked = res['filters.skipFiltered'] !== false;
}
