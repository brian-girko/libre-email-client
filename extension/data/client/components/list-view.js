...
class ListView extends HTMLElement {
      ...
      <dialog class="move-dialog after-save-dialog">
        <form>
          ...
          <div class="row">
            ...
            <button type="button" class="delete">Mark \Deleted</button>
            <button class="ok">Move</button>
          </div>
        </form>
      </dialog>`;
  ...
  // Ask what to do with the server originals after a native save (the
  // "Actions" override ran without its "Delete from IMAP" checkbox).
  // Resolves 'deleted' (mark \Deleted, expunge when the core supports it),
  ...
  askAfterSave(count, label) {
    ...
    const turn = this.#afterSaveQueue.then(() => new Promise(resolve => {
      ...
      this.#afterSaveSub.textContent =
        'What should happen to the originals on the server? "Move" transfers them to the server\u2019s ' + label + ' folder.';
  ...
  removeRows(uids) {
    ...
    const kept = [];
    for (const thread of this.#rows) {
      const messages = thread.messages.filter(item => {
        if (wanted.has(Number(item.uid))) {
    ...
    this.#rows = kept;
    this.#expanded = new Set([...this.#expanded].filter(key => this.#rows.some(t => t.uids[0] === key)));
    this.#selected = new Set([...this.#selected].filter(uid => !wanted.has(Number(uid))));
    // the server-side count shrinks by the removed messages; keep the pager
    // totals honest after optimistic deletions
    if (removed.length && this.#pager) {
      this.#pager.total = Math.max(0, (this.#pager.total || 0) - removed.length);
    }
    // re-derive the selection from the active filter so selection == matches
    // still holds after rows were removed
    this.#applyFilter(this.#filterQuery);
    if (removed.length) {
      this.dispatchEvent(new CustomEvent('email-gone', {
        detail: {uids: removed},
  ...
  // Deep-copy the threads that contain any of the given uids, for rollback
  // after an optimistic removal. Captured before removeRows() so each thread
  // is intact including messages that will be deleted.
  snapshotRows(uids) {
    const wanted = new Set((Array.isArray(uids) ? uids : []).map(Number));
    const list = [];
    for (const thread of this.#rows) {
      if (thread.messages.some(m => wanted.has(Number(m.uid)))) {
        list.push(structuredClone(thread));
      }
    }
    return list;
  }

  // Undo removeRows() with the snapshots taken before it: threads that were
  // partly emptied get their messages back, fully dropped threads are
  // re-inserted, then the view is re-rendered through the normal filter path.
  restoreRows(snapshots) {
    for (const snap of snapshots) {
      const key = snap.uids[0];
      const existing = this.#rows.find(t => t.uids[0] === key);
      if (existing) {
        existing.messages = snap.messages;
  ...
  #recalc(thread) {
    thread.uids = thread.messages.map(m => m.uid);
